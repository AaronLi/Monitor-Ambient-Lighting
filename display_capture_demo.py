import cv2
from pygame import *

from threaded_image_grabber import sct, ThreadedImageGrabber

screen = display.set_mode((sct.monitors[0]['width'] // 2, sct.monitors[0]['height'] // 2))

image_grabber = ThreadedImageGrabber().start()

clockity = time.Clock()
while image_grabber.is_running:
    for e in event.get():
        if e.type == QUIT:
            image_grabber.stop()

    captured_screenshot = image_grabber.pull_frame_and_request_next()
    if captured_screenshot is None:
        continue
    captured_screenshot = cv2.cvtColor(
        cv2.resize(captured_screenshot, (captured_screenshot.shape[1] // 64, captured_screenshot.shape[0] // 64)),
        cv2.COLOR_BGR2RGB)

    out_image = transform.flip(transform.rotate(surfarray.make_surface(captured_screenshot), -90), True, False)

    screen.blit(transform.scale(out_image, screen.get_size()), (0, 0))
    clockity.tick(30)
    display.flip()

    display.set_caption(f'FPS: {clockity.get_fps():.2f}')

quit()
